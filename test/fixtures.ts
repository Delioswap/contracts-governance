...
export async function governanceFixture(
  ...
  // deploy DSP, sending the total supply to the deployer
  const { timestamp: now } = await provider.getBlock('latest')
  const timelockAddress = Contract.getContractAddress({ from: wallet.address, nonce: 1 })
  const dsp = await deployContract(wallet, Dsp, [wallet.address, timelockAddress, now + 60 * 60])
  ...
  const governorAlpha = await deployContract(wallet, GovernorAlpha, [timelock.address, dsp.address])
  expect(governorAlpha.address).to.be.eq(governorAlphaAddress)

  return { dsp: dsp, timelock, governorAlpha }
}
