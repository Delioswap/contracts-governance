...
import DelioswapPair from '@delioswap/core/build/DelioswapPair.json'
...
describe('scenario:FeeTo', () => {
  ...
  beforeEach('deploy delioswap', async () => {
    factory = await deployContract(wallet, DelioswapFactory, [wallet.address])
  })
  ...
  describe('tokens', () => {
    ...
    beforeEach('make test tokens', async () => {
      const { timestamp: now } = await provider.getBlock('latest')
      const token0 = await deployContract(wallet, Dsp, [wallet.address, constants.AddressZero, now + 60 * 60])
      tokens.push(token0)
      const token1 = await deployContract(wallet, Dsp, [wallet.address, constants.AddressZero, now + 60 * 60])
      tokens.push(token1)
    })
    ...
    beforeEach('create fee liquidity', async () => {
      ...
      pair = new Contract(pairAddress, DelioswapPair.abi).connect(wallet)
