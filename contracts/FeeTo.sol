...
contract FeeTo {
    ...
    function renounce(address pair) public returns (uint value) {
        ...
        TokenAllowState storage token0AllowState = tokenAllowStates[IDelioswapPair(pair).token0()];
        TokenAllowState storage token1AllowState = tokenAllowStates[IDelioswapPair(pair).token1()];
        ...
        if (
            ...
            value = IDelioswapPair(pair).balanceOf(address(this));
            if (value > 0) {
                ...
                assert(IDelioswapPair(pair).transfer(pair, value));
                IDelioswapPair(pair).burn(pair);
            }
    ...
    function claim(address pair) public returns (uint value) {
        ...
        if (
            ...
            if (value > 0) {
                ...
                assert(IDelioswapPair(pair).transfer(feeRecipient, value));
            }
        }
    }
}
